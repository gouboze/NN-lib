...
exports.Neuron.prototype.adjustWeights = function (momentum, trainingStep, prevLayer) {
  ...
  if (prevLayer !== {}) {
    for ( var i = 0; i < this.curWeights.length; i++) {
      this.nextWeights[i].value = this.curWeights[i].value + momentum * (this.curWeights[i].value - this.prevWeights[i].value) + trainingStep * this.delta * prevLayer.neurons[i].outputValue;
    }
  }
};
